...
#define PID_FREE   0 //the process can be used
...
pid_t new_pid() {
    int spl = splhigh();
    // if there isn't any recently freed process ids
    if (recycled_pids == NULL) {
        // assert there is space left to make a new one
        ...
        //advance the upper index of the unused_pid list
        unused_pids += 1;
        splx(spl);
